<#
.SYNOPSIS
  nini-agents.ps1 -- Run multiple sandboxed profiles of supported CLIs, IDEs, and GUI apps.
...
.DESCRIPTION
  ...
  USAGE
    ...
    nini-agents doctor                 Diagnose environment
    nini-agents help                   Full command reference
#>
...
function Resolve-MultiCliModulePath {
    ...
    if (-not (Test-Path -LiteralPath $modulePath)) {
        throw "Module '$ModuleName' not found for $ToolsDir. Reinstall nini-agents."
    }
    return $modulePath
}
...
function Get-Adapter {
    ...
    if (-not (Test-Path $manifest)) { throw "Unknown tool '$ToolId'. Run: nini-agents tools" }
...
function Invoke-Auth {
    ...
    switch ($Action) {
        ...
        default { throw 'Usage: nini-agents auth <set|status|clear> <tool>/<profile>' }
    }
}
...
function Invoke-Continue {
    ...
    if (-not $Tool -or -not $SrcName -or -not $DestName) {
        throw "Usage: nini-agents continue <tool> <src-profile> <dest-profile> [--no-merge] [--dry-run]"
    }
    ...
    if ((Get-ObjectPropertySafe -Object $adapter -Name 'schemaVersion') -eq 2) {
        ...
        if (-not (Test-Path $destDir)) {
            throw "Destination profile '$DestName' does not exist. Create it with: nini-agents new $Tool/$DestName"
        }
...
function Initialize-SessionSeed {
    ...
    if ($bytes -gt $SEED_MAX_BYTES) {
        ...
        Write-Host "base session state is $(Format-Bytes $bytes); skipped automatic copy -- run: nini-agents continue $($Adapter.id) base $name"
        return 0
    }
    $credentials = @($Adapter.session.credentials)
    $copied = 0; $skipped = 0
...
function Throw-LegacyTransferBlocked {
    param([string]$Action, [string]$Spec)
    throw "Cannot $Action '$Spec': legacy profile transfer is disabled because whole-root copies can leak tokens. Migrate the legacy profile first: nini-agents migrate $Spec"
}
...
# nini-agents new <tool>/<name>: create the profile dir, seed from base unless
...
function New-StartMenuShortcut {
    ...
    try {
        $linkName = "nini-agents $Tool $Name.lnk"
...
function Remove-StartMenuShortcut {
    param([string]$Tool, [string]$Name)
    $linkPath = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\nini-agents $Tool $Name.lnk"
    if (Test-Path $linkPath) { Remove-Item -Force $linkPath }
    $legacyLinkPath = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\multi-cli $Tool $Name.lnk"
    if (Test-Path $legacyLinkPath) { Remove-Item -Force $legacyLinkPath }
}
...
function Invoke-LaunchSandboxUser {
    ...
    if (-not (Get-LocalUser -Name $username -ErrorAction SilentlyContinue)) {
        ...
        if (-not $isAdmin) {
            throw "First launch requires admin. Run as Administrator: nini-agents launch $($Adapter.id)/$profileName"
        }
...
function Show-Doctor {
    param([string]$Deep)
    $errors = 0; $warnings = 0
    Write-Host "nini-agents $VERSION  --  Windows"
    Write-Host ""
...
# nini-agents template save|list|delete: legacy profiles copy wholesale minus
...
function Invoke-Template {
    ...
    switch ($Sub) {
        'save' {
            if (-not $A -or -not $B) { throw "Usage: nini-agents template save <tool>/<profile> <name>" }
        ...
        'delete' {
            if (-not $A) { throw "Usage: nini-agents template delete <name>" }
            Test-ProfileName $A
        ...
        default { throw "Usage: nini-agents template <save|list|delete>" }
    }
}

# nini-agents export: schema-v2 profiles export via the allowlist transfer;
...
# nini-agents import: schema-v2 archives are validated and re-identified by the
...
function Invoke-Import {
    ...
    if (-not $Spec) { throw "Usage: nini-agents import <archive> <tool>/<name>" }
    $p = Split-ProfileSpec $Spec
    Test-ProfileName $p.Name
    $adapter = Get-Adapter $p.Tool
...
function Show-Help {
@"
nini-agents $VERSION -- sandboxed profiles for CLIs, IDEs, and GUI apps
...
@"
Register-ArgumentCompleter -Native -CommandName nini-agents,multi-cli -ScriptBlock {
...
function Read-NewFlags {
    ...
    for ($i = 0; $i -lt $Tokens.Count; $i++) {
        switch ($Tokens[$i]) {
            ...
            default { throw "Unknown option for new: '$($Tokens[$i])'. Run: nini-agents help" }
        }
    }
...
try {
    switch ($Cmd) {
        ...
        '--help'    { Show-Help }
        '-h'        { Show-Help }
        'version'   { Write-Host "nini-agents $VERSION" }
        '--version' { Write-Host "nini-agents $VERSION" }
        '-v'        { Write-Host "nini-agents $VERSION" }
        ''          { Show-Help; exit 1 }
        default {
            if ($Cmd -match '/') {
                $forward = @()
