<p align="center">
  <img src="../../assets/i18n/zh/banner.svg" alt="Nini Agents。无需切换，同时使用多个账户。" width="760"/>
</p>
...
平台限制请参阅[支持矩阵](../support-matrix.md)。运行 `nini-agents tools` 可检查本机环境。
...
| 命令 | 操作 |
|---|---|
| `nini-agents new <tool>/<name>` | 创建凭据分离、正常状态共享的账户配置文件 |
| `nini-agents new <tool>/<name> --isolated` | 在 AI 工具支持时创建整个主目录均隔离的配置文件；别名：`--isolate`、`-i` |
| `nini-agents new <tool>/<name> --from <template>` | 从 schema v2 模板创建 schema v2 配置文件 |
| `nini-agents <tool>/<name>` | 启动配置文件 |
| `nini-agents launch <tool>/<name> [-- args...]` | 启动工具并向其传递参数 |
| `nini-agents list [<tool>]` | 列出配置文件 |
| `nini-agents clone <tool>/<src> <tool>/<dest>` | 复制 schema v2 配置文件 |
| `nini-agents rename <tool>/<old> <tool>/<new>` | 重命名配置文件 |
| `nini-agents delete <tool>/<name>` | 确认后删除配置文件 |
...
| `nini-agents export <tool>/<name> [path]` | 导出 schema v2 配置文件 |
| `nini-agents import <archive> <tool>/<name>` | 导入 schema v2 归档 |
...
| `nini-agents completion {bash\|zsh\|powershell}` | 输出 shell 补全设置 |
| `nini-agents help` | 显示所有命令 |
| `nini-agents version` | 显示已安装的版本 |
...
配置文件使用最窄的受支持边界。`--isolated` 会创建独立的工具主目录。固定的操作系统凭据会使用由 Nini Agents 管理的操作系统用户，并且在 Windows 上需要提升权限的终端。
...
| `NINI_AGENTS_REPO` | GitHub 仓库 | 覆盖安装来源 |
| `NINI_AGENTS_INSTALL_DIR` | 平台默认值 | 覆盖安装目录 |

`MULTICLI_REPO` 和 `MULTICLI_INSTALL_DIR` 暂时保留为兼容别名。
`multi-cli` 命令也暂时作为 `nini-agents` 引擎的兼容入口。
...
如果安装后找不到 `nini-agents` 或新配置文件的别名，请重启终端。[支持矩阵](../support-matrix.md)列出了各产品的具体要求。
