...
Describe 'Test-AdapterManifest schema-v2 paths and separation in-process' {
    Add-ManifestCases @(
        ...
        @{ Name = 'flags shared paths overlapping session paths'; Base = 'v2'; Id = 'fixture'; Mutate = { param($a) $a.normalState.sharedPaths = @('state'); $a.normalState.sessionPaths = @('state/sessions') }; Count = 1; Messages = @("shared path 'state' overlaps session path 'state/sessions'") },
