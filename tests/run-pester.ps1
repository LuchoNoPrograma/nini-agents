...
.DESCRIPTION
  ...
  a test has an unexpected skip, is pending, or is inconclusive. Requires Pester 3.x (>=3.4,
  <4).
#>
...
if ($Coverage) {
    & (Join-Path $here 'coverage\Invoke-ModuleCoverage.ps1') -MinimumPercent $MinimumPercent -TestPath $Path
    exit $LASTEXITCODE
}
. (Join-Path $here 'helpers\pester-policy.ps1')
...
$policy = Get-PesterRunPolicy -Result $result -Report
if ($result.FailedCount -gt 0 -or $result.TotalCount -eq 0 -or ($CI -and -not $policy.passed)) { exit 1 }
