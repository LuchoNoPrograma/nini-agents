...
- Entradas `multi-cli` y `multi-cli.ps1`, modulos de `lib/`, definiciones de `ai-tools/`, pruebas de `tests/` y documentacion contractual de `docs/`.
...
- `multi-cli` - `cmd_new` y `multi-cli.ps1` - `New-Profile`: comandos equivalentes conservan validacion, metadata y comportamiento por plataforma.
...
- No modificar la instalacion activa bajo `~/.local/share/multi-cli` durante el desarrollo.
